  ...
  const handleGoogleSuccess = async (tokenResponse) => {
    try {
      ...
      if (res && res.requires_phone) {
          // The backend indicates this is a new Google signup and Needs a phone number
          setForm(prev => ({ 
              ...prev, 
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111827', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          >
            {loading ? 'Please wait...' : (tab === 'login' ? 'Login' : (tab === 'register' ? 'Register' : 'Complete Profile'))}
          </button>
        </form>
