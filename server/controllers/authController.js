...
exports.loginRadiusAuth = async (req, res) => {
  const { access_token } = req.body;
  if (!access_token) {
    return res.status(400).json({ success: false, message: 'Access token is required.' });
  }
  ...
  try {
    // 1. Fetch user details from LoginRadius API using the access token
    const API_KEY = process.env.LOGINRADIUS_API_KEY || '5330c4fd-bd0e-4fc7-9df7-bb63ef0dec58';
    
    const lrRes = await fetch(`https://api.loginradius.com/identity/v2/auth/account?apikey=${API_KEY}`, {
      headers: { Authorization: `Bearer ${access_token}` }
    });
    
    if (!lrRes.ok) {
      const errText = await lrRes.text();
      console.error('LoginRadius Auth Error Response:', errText);
      return res.status(401).json({ success: false, message: 'Invalid LoginRadius access token.' });
    }

    const lrProfile = await lrRes.json();
    
    // LoginRadius returns phone numbers in PhoneId, emails in Email[0].Value
    let cleanEmail = null;
    let cleanPhone = null;
    let cleanName = normalizeName(lrProfile.FullName || lrProfile.FirstName || 'User');
    
    if (lrProfile.Email && lrProfile.Email.length > 0) {
      cleanEmail = normalizeEmail(lrProfile.Email[0].Value);
    }
    
    if (lrProfile.PhoneId) {
       cleanPhone = lrProfile.PhoneId;
    }

    // We must have at least an email or phone to identify the user
    if (!cleanEmail && !cleanPhone) {
        return res.status(400).json({ success: false, message: 'No email or phone number provided by authentication provider.' });
    }

    // Determine query strategy (since phone column may not exist depending on the migration state, we default to treating phone logins as a pseudo-email if strictly required, or querying by email if we have it)
    
    // Check if user exists primarily by email. If they only signed up with phone, we use a fake email since Postgres requires it currently
    const dbEmailToUse = cleanEmail || `${cleanPhone}@phone.local`;
    ...
    if (result.rows.length === 0) {
      // User doesn't exist, create a new one. Password is null since they use LoginRadius.
      const insertResult = await db.query(
        `INSERT INTO users (name, email, role)
         VALUES ($1, $2, $3)
         RETURNING id, name, email, role, created_at`,
        [cleanName, dbEmailToUse, 'user']
      );
      user = insertResult.rows[0];
      statusCode = 201;
    ...
    console.error('LoginRadius Auth Error:', error);
    return res.status(500).json({
      success: false,
